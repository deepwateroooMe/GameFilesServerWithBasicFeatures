...
- 
- 
* root@localhost连不上mysql权限问题的设置步骤
- window 10 终端root@localhost连不上mysql: 解决主要步骤:
- 打开MySQL目录下的my.ini文件，删除最后一行的“skip-grant-tables”，保存并关闭文件。这里有个另包的小步骤是需要修改这个文件的安全权限
- 终端必要的权限设置：
#+BEGIN_SRC shell
...
[[./pic/readme_20230111_162732.png]]
...
- 数据库功能模块能够使用visual studio 2019 版本连接MySQL数据库创建修改表格等
- 登录与登出系统,仍在试图去理解为什么视图加载时无法动态检测我还没有登录(没有显示登录按钮)[这个小bug已经解决，将关于用户帐户登录与否的部分视图添加到视图中去].
- 这个部分还有一个小bug，晚点儿再来修复它(_LoginPartial.cshtml null ==> "/")。wired way of fixing it. 
- 现在只是个文件服务器,只需要对用户登录等帐户系统进行一定的管理,游戏数据呢?不是要把用户游戏数据上传至服务器,给用户双保险的吗?
- 现在,对这个服务器端的数据库的设计还没有整理清楚.可以考虑采用别人已经封装得比较好一点儿的第三方库,比如 https://github.com/praeclarum/sqlite-net
- 用visual studio 2019,可以连上自己电脑上的MySQL数据库,并获取相应的表格等基本操作.那么现在就是如何把数据库与服务器上的用户的登录等绑定起来(那么这个简单的服务器就会成为传说中的游戏登录服,主要负责游戏用户的登录以及用户登录相关的后台数据库的处理连接)(但是感觉还需要一个真正可以运行起来的github上别人的项目来给自己一个完整的概念) 
  - 上面2019连MySQL没有问题.找到一个参考项目,但还有两荐配置还没弄太懂,要配置好能够运行起来的,才能把这个功能模块补完
  - https://github.com/AA-PastProjects/LoginAndRegistrationAPI
- 现在可以 *兵分两路的设计与实现这个最简单的服务器: 登录服相关的(登录+数据库)*, 处理前面提到过的自己根据同自己项目的熟悉程度想设计出来的,服务器如何缓存一资源包的MD5码表(这一步仍然可以当作之后的优化,之后再处理),必须先实现的是, *当手机平台客户端,请求资源下载时(一个或是几个资源包),服务器如何处理相关逻辑来实现客户端所请求资源包文件的下载到客户端.*
- 早上会再回去读一下ET框架中登录服相关的逻辑
- 会抽时间再总结 review一下之前项目中关于VR眼镜产品的customized Android OS system, 相关application, 3D 视频播放相关的OpenGL rendering性能优化等相关部分,将之前做过的项目再review一遍,在缺少测试工具(没有VR产品,没有可测试工具)的情况下,看自己仍然能够消化或总结出哪些主要相关功能模块
